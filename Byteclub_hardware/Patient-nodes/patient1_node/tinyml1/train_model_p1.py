...
X, y = [], []
with open("vitals_dataset_p1.csv") as f:
    reader = csv.DictReader(f)
    for row in reader:
        X.append([float(row["hr"]), float(row["spo2"]), float(row["temp"]), float(row["humidity"])])
...
print("=== PATIENT 1 MODEL: Per-condition sensitivity ===")
...
for i, cls in enumerate(CLASSES):
    ...
    print(f"  {cls:15s}: {correct}/{total} = {sens*100:.1f}%{tag}")
...
with open("model_p1.pkl", "wb") as f:
    pickle.dump(clf, f)
print(f"\nSaved -> model_p1.pkl | Tree depth: {clf.get_depth()}, leaves: {clf.get_n_leaves()}")
